//UCTS_ShadingGradeMap_VerticalDissolve.cginc
...
//#include "UCTS_ShadingGradeMap_VerticalDissolve.cginc"
            ...
            //Emissive
            UNITY_DECLARE_TEX2DARRAY(_Emissive_Tex_And_Border_Tex);
...
//
            ...
            float4 frag(VertexOutput i) : SV_TARGET {
                ...
                float colorMask_ = mask2 * (1.0 - _InvertMask) + _InvertMask * (1.0 - mask1);
			    clip(opacityMask - 0.5);

                //VerticalDissolve Border
			    float border = mask1 - mask2;
                float4 borderTexture = UNITY_SAMPLE_TEX2DARRAY(_Emissive_Tex_And_Border_Tex, float3(Set_UV0, 1));
			    float4 coloredBorder = borderTexture * border * _BorderColor;

                //VerticalDissolve
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(Set_UV0, _MainTex)) * colorMask_;
//v.2.0.4
#ifdef _IS_TRANSCLIPPING_OFF
//
#elif _IS_TRANSCLIPPING_ON
...
////// Lighting:
                ...
                //VerticalDissolve
                float4 _1st_ShadeMap_var = lerp(tex2D(_1st_ShadeMap,TRANSFORM_TEX(Set_UV0, _1st_ShadeMap)),_MainTex_var,_Use_BaseAs1st) * colorMask_;
                ...
                //VerticalDissolve
                float4 _2nd_ShadeMap_var = lerp(tex2D(_2nd_ShadeMap,TRANSFORM_TEX(Set_UV0, _2nd_ShadeMap)),_1st_ShadeMap_var,_Use_1stAs2nd) * colorMask_;
                ...
                //VerticalDissolve
                float4 _Emissive_Tex_var = UNITY_SAMPLE_TEX2DARRAY(_Emissive_Tex_And_Border_Tex, float3(Set_UV0, 0)) * _Emissive_Color * colorMask_;
//v.2.0.4
#ifdef _IS_ANGELRING_OFF
...
#endif
                finalColor = saturate(finalColor + DecodeLightProbe(normalDirection)*_GI_Intensity) + (_Emissive_Tex_var.rgb + coloredBorder);//Final Composition
